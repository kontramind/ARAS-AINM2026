"""
run_ensemble_tta.py  —  NorgesGruppen Submission: Two-Model Ensemble + TTA
--------------------------------------------------------------------------
Runs two ONNX models (YOLOv8x@960 + YOLOv8l@640) on each image.
Optionally applies horizontal-flip TTA per model.
All predictions (2 or 4 passes per image) are fused with Weighted Boxes Fusion.

Usage:
    python run_ensemble_tta.py --input /data/images --output /output/predictions.json
...
Files expected in same directory as this script:
    best.onnx       — primary model   (YOLOv8x@960, FP16)
    best_l.onnx     — secondary model (YOLOv8l@640, FP16)
    best.json       — metadata sidecar for primary   (optional)
    best_l.json     — metadata sidecar for secondary (optional)
"""
...
MODEL_A  = _HERE / "best.onnx"      # primary:   YOLOv8x@960
MODEL_B  = _HERE / "best_l.onnx"   # secondary: YOLOv8l@640

PROVIDERS = ["CUDAExecutionProvider", "CPUExecutionProvider"]

# ─── Inference config ─────────────────────────────────────────────────────────
CONF_THRES  = 0.05    # pre-WBF gate (low — WBF handles duplicates)
WBF_IOU     = 0.55
WBF_SKIP    = 0.001
TTA_FLIPLR  = True    # horizontal-flip TTA (adds 2 more passes per image)
...
def decode(raw: np.ndarray, orig_w: int, orig_h: int,
           ...
           fliplr: bool = False):
    """
    Decode raw ONNX [1, 4+nc, A] output → normalised [0,1] WBF-ready boxes.
    Un-mirrors x-coords when fliplr=True.
    """
    pred    = raw[0].T.astype(np.float32)     # [A, 4+nc]
    bxs     = pred[:, :4]                     # cx, cy, w, h
    cls_sc  = pred[:, 4:]
...
def main():
    ...
    for mp in (model_a_path, model_b_path):
        if not mp.exists():
            raise FileNotFoundError(
                ...
                "Ensure best.onnx and best_l.onnx are next to run.py."
            )
    ...
    n_passes = len(flips) * 2   # 2 models × flips

    print(f"[ensemble] Provider  : {sess_a.get_providers()[0]}")
    print(f"[ensemble] Model A   : {model_a_path.name}  imgsz={imgsz_a}")
    print(f"[ensemble] Model B   : {model_b_path.name}  imgsz={imgsz_b}")
    print(f"[ensemble] TTA fliplr: {use_tta}  ({n_passes} passes/image)")
    print(f"[ensemble] Images    : {len(image_paths)}")
    ...
    for i, img_path in enumerate(image_paths):
        ...
        for (sess, inp_name, inp_dtype, out_names, imgsz) in models:
            for fliplr in flips:
        ...
        if (i + 1) % 10 == 0 or (i + 1) == n:
            print(f"[ensemble] {i+1}/{n} done …")

    print(f"[ensemble] Total predictions : {len(all_predictions)}")
